    ...
    methods: {
      ...
      dialogAdd () {
        axios.post('/user/signup', {
          // 上传的数据如下
          user: this.add.user, // 账号(学号)
          password: this.add.password, // 密码
          name: this.add.name, // 姓名
          // email: this.add.email
          tel: this.add.tel, // 手机号
          grade: this.add.grade, // 年级
          classNo: this.add.classNo, // 班级
          major: this.add.major // 专业
        })
          .then(response => {
      ...
      getStudentsInfo (condition = {}, skipto = 0, limit = 10, sortby = '-_id') {
        axios.post('/teacher/students', {
          ...
          .then(response => {
            ...
            this.studentData = response.data.data.usersInfo || undefined // 把得到的数据返回给studentData给表格显示
            this.totalPage = response.data.data.countUser || 0 // 所有总的数据数目
          })
          .catch(error => {
            console.log('fail' + error)
            this.$notify.error({
